package com.icbc.api;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.EnterpriseOpenpayPaymentxRequestV2;
import com.icbc.api.request.EnterpriseOpenpayPaymentxRequestV2.EnterpriseOpenpayPaymentxRequestV2Biz;
import com.icbc.api.request.EnterpriseOpenpayPaymentxRequestV2.EnterpriseOpenpayPaymentxRequestV2Biz.EnterPriseOpenpayPaymentxGoodInfo;
import com.icbc.api.request.EnterpriseOpenpayPaymentxRequestV2.EnterpriseOpenpayPaymentxRequestV2Biz.EnterPriseOpenpayPaymentxRecInfo;
import com.icbc.api.response.EnterpriseOpenpayPaymentxResponseV2;

public class EnterpriseOpenpayPaymentxTest {

	/**
	 * @param args
	 */

	protected static final String MY_PRIVATE_KEY = "";

	protected static final String APIGW_PUBLIC_KEY = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "0200EH0005040"; 

	protected static final String password = "12345678";

	protected static final String MY_PUB_KEY = "";

	public static void main(String[] args) {
		// TODO 自动生成的方法存根

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				MY_PRIVATE_KEY, APIGW_PUBLIC_KEY, MY_PUB_KEY, password);

		// 设置请求对象request
		EnterpriseOpenpayPaymentxRequestV2 request = new EnterpriseOpenpayPaymentxRequestV2();

		// 设置请求路径
		request.setServiceUrl("https://ip:port/api/enterprise/openpay/paymentx/V2");

		EnterpriseOpenpayPaymentxRequestV2Biz bizContent = new EnterpriseOpenpayPaymentxRequestV2Biz();

		bizContent.setPay_name("李尧集团人民币户");
		bizContent.setPay_rem("payremark");
		bizContent.setGet_phone("13426351013");
		bizContent.setTrade_time("20211010162925");
		bizContent.setGet_address("get_address");
		bizContent.setFile_code("0200EH0011036");
		bizContent.setPay_acct_num("0200062009213084848");
		bizContent.setOrder_no("orderzydl0050");
		bizContent.setGet_county("Qianshan");
		bizContent.setPay_phoneno("010-82706722");
		bizContent.setLanguage("ZH_CN");
		bizContent.setFile_serialno("fsZHJlyd0049");
		bizContent.setTradeplat_code("2018070000000000035");
		bizContent.setPay_mode("0");
		bizContent.setAmout("250");
		bizContent.setAcct_orgcode("12345");
		bizContent.setChannel_flag("0");
		bizContent.setGet_post("100101");
		bizContent.setOrder_rem("订单备注test");
		bizContent.setCurrtype("001");
		bizContent.setFile_name("蜇惕艘厦潭配状滥务蛭迪参扮戒芒台");
		bizContent.setSign_flag("1");
		bizContent.setTradeplat_rem("tradeplat_rem");
		bizContent.setGet_email("www@139.com");
		bizContent.setGet_city("Anhui");
		bizContent.setTrade_orgcode("91350922MA34A6CP84");
		bizContent.setTradeplat_name("李尧集团基本户");
		bizContent.setGet_province("Anhui");
		bizContent.setFile_id("0200EH0011036.e.0200");
		
		List<EnterPriseOpenpayPaymentxRecInfo> reclist = (List)new ArrayList<EnterPriseOpenpayPaymentxRecInfo>();
		
		EnterPriseOpenpayPaymentxRecInfo recinfo1 = new EnterPriseOpenpayPaymentxRecInfo();
		EnterPriseOpenpayPaymentxRecInfo recinfo2 = new EnterPriseOpenpayPaymentxRecInfo();
		EnterPriseOpenpayPaymentxRecInfo recinfo3 = new EnterPriseOpenpayPaymentxRecInfo();
		//收款人1
		recinfo1.setRec_seqno("1");
		recinfo1.setRec_name("李四");
		recinfo1.setRec_acct_num("6222020200066551234");
		recinfo1.setRec_bnkclscode("12");
		recinfo1.setRec_orgcode("1234567890");
		recinfo1.setSystem_flag("1");
		//收款人2
		recinfo2.setRec_seqno("2");
		recinfo2.setRec_name("李四");
		recinfo2.setRec_acct_num("6222020200066551234");
		recinfo2.setRec_bnkclscode("12");
		recinfo2.setRec_orgcode("1234567890");
		recinfo2.setSystem_flag("1");
		//收款人3
		recinfo3.setRec_seqno("3");
		recinfo3.setRec_name("李四");
		recinfo3.setRec_acct_num("6222020200066551234");
		recinfo3.setRec_bnkclscode("12");
		recinfo3.setRec_orgcode("1234567890");
		recinfo3.setSystem_flag("1");
		
		reclist.add(recinfo1);
		reclist.add(recinfo2);
		reclist.add(recinfo3);

		List<EnterPriseOpenpayPaymentxGoodInfo> goodlist = new ArrayList<EnterPriseOpenpayPaymentxGoodInfo>();
		EnterPriseOpenpayPaymentxGoodInfo goodinfo1 = new EnterPriseOpenpayPaymentxGoodInfo();
		goodinfo1.setGoods_name("苹果");
		goodinfo1.setGoods_num("11");
		goodinfo1.setGoods_seqno("SP001");
		goodinfo1.setPrice("850");
		goodinfo1.setSeller_phoneno("13811111111");
		goodinfo1.setSeller_name("appler_seller");
		goodinfo1.setUnits("KG");
		goodinfo1.setWeight("3");
		
		EnterPriseOpenpayPaymentxGoodInfo goodinfo2 = new EnterPriseOpenpayPaymentxGoodInfo();
		goodinfo2.setGoods_name("香蕉");
		goodinfo2.setGoods_num("11");
		goodinfo2.setGoods_seqno("SP002");
		goodinfo2.setPrice("850");
		goodinfo2.setSeller_phoneno("13811111111");
		goodinfo2.setSeller_name("appler_seller");
		goodinfo2.setUnits("KG");
		goodinfo2.setWeight("3");
		
		goodlist.add(goodinfo1);
		goodlist.add(goodinfo2);
		
		bizContent.setReclist(reclist);
		bizContent.setGoodslist(goodlist);
		
		request.setBizContent(bizContent);
		EnterpriseOpenpayPaymentxResponseV2 response;

		Random rand = new Random();
		String msgId = rand.nextInt(99999) + "msg";
		System.out.println(msgId);

		try {
			response = client.execute(request, msgId);
			if (response.getReturnCode() == 0) {
				// 业务成功处理
				System.out.println("success");//
			} else {
				// 失败
				System.out.println("error");
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		} finally {
		}
	}
}