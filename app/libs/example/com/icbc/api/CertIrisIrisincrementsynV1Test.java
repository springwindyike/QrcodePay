...
import com.icbc.api.request.CertIrisIrisincrementsynRequestV1;
import com.icbc.api.request.CertIrisIrisincrementsynRequestV1.CertIrisIrisincrementsynRequestV1Biz;
import com.icbc.api.response.CertIrisIrisincrementsynResponseV1;


public class CertIrisIrisincrementsynV1Test {

	private static final String APP_ID = new String("100000a00000004095737");
//	调用方私钄1�7
...
//		签名类型为SHA256WithRSA时，传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省�1�7�1�7
		...
		CertIrisIrisincrementsynRequestV1 request = new CertIrisIrisincrementsynRequestV1();

//		测试环境霄1�7要替换ip和端口，生成环境已有默认值，此语句可删除
		request.setServiceUrl(API_SERVICE_URL +"/cert/iris/irisincrementsyn/V1");
//		请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋倄1�7
		CertIrisIrisincrementsynRequestV1Biz bizContent = new CertIrisIrisincrementsynRequestV1Biz ();
//		
		bizContent.setChannel("101");
		...
		CertIrisIrisincrementsynResponseV1 response = new CertIrisIrisincrementsynResponseV1();
		Logger log = Logger.getLogger(CertIrisIrisincrementsynV1Test.class.getName());
		try {
			...
			if (response.isSuccess()) {
				...
				log.info("ReturnCode:" + response.getReturnCode());
				log.info("ReturnMsg:" + response.getReturnMsg());
				log.info("exMsg:" + response.getExMsg());
			}
