package com.icbc.api;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.CertCardauthIcbccardsmsverifyRequestV1;
import com.icbc.api.request.CertCardauthIcbccardsmsverifyRequestV1.CertCardauthIcbccardsmsverifyBiz;
import com.icbc.api.response.CertCardauthIcbccardsmsverifyResponseV1;

import java.text.SimpleDateFormat;
import java.util.logging.Logger;


public class CertCardauthIcbccardsmsverifyTestV1 {
    public static void main(String[] args) throws IcbcApiException {

        //1��APIƽ̨�����APPID
        String APP_ID = "1000000000000BCSS4095733";

        //2�����ع�Կ
        String APIGW_PUBLIC_KEY = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //3����Կ����֤��ʽ����Կ��APIƽ̨�Ǽǣ��˴�Ϊ˽Կ
        String MY_PRIVATE_KEY = "";

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CertCardauthIcbccardsmsverifyRequestV1 request = new CertCardauthIcbccardsmsverifyRequestV1();

        //4��API���ص�ַ�����ݲ��Ի��������������滻��ӦIP�Ͷ˿�
        //request.setServiceUrl("http://122.64.61.111:8081/api/cert/cardauth/icbccardsmsverify/V1");//����
        request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cert/cardauth/icbccardsmsverify/V1");//��������
		
        
        //5������սӿ��ĵ���bizContent.setxxx()������ҵ���������ݽ��и�ֵ
        CertCardauthIcbccardsmsverifyBiz bizContent = new CertCardauthIcbccardsmsverifyBiz();

        SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
        SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm:ss");
        long dataNum = System.currentTimeMillis();

        bizContent.setTrxSerno("1020BCSS811111456141");
        bizContent.setTrxDate(sdfDate.format(dataNum));
        bizContent.setTrxTime(sdfTime.format(dataNum));
        bizContent.setPhoneNo("18820423459");//�ֻ���
        bizContent.setVerifyCode("352484");//��̬��
        bizContent.setVerifyCodeNo("574935");//��̬����

        request.setBizContent(bizContent);
        CertCardauthIcbccardsmsverifyResponseV1 response;
        try {
            String msg_id = String.valueOf(System.currentTimeMillis());
            response = client.execute(request, msg_id);//msgId��ϢͨѶΨһ��ţ�Ҫ��ÿ�ε��ö������ɣ�APP��Ψһ
            Logger log = Logger.getLogger(CertCardauthIcbccardsmsverifyTestV1.class.getName());
            if (response.isSuccess()) {
            log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
            } else {	 
            log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
            }
            //System.out.println("ReturnCode:" + response.getReturnCode());
           // System.out.println("ReturnMsg:" + response.getReturnMsg());
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }

}
