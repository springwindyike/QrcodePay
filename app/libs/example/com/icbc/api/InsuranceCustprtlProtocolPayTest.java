package com.icbc.api;

import com.icbc.api.request.InsuranceCustprtlProtocolPayRequestV1;
import com.icbc.api.response.InsuranceCustprtlProtocolPayResponseV1;

import java.util.logging.Logger;

public class InsuranceCustprtlProtocolPayTest {
    static Logger log = Logger.getLogger(InsuranceCustprtlProtocolPayTest.class.getName());
    //估值表
    protected static String APP_ID = "xxxxxxxxxxxxxx";
    protected static String BASE_URL = "http://ip:port/api"; // 行外

    //网关公钥
    protected static String APIGW_PUBLIC_KEY = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用私钥（与内管平台注册app时的“签名公钥”配对）
    protected static String PRI_KEY = "";

    public static void main(String[] args) throws Exception {
        String URI = "mybank/pay/insurance/primum/charge/V1"; // api地址

        //DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
        DefaultIcbcClient client =  new DefaultIcbcClient(APP_ID, "RSA2", PRI_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, "AES", "EZNm07FGm6wtbdP7kKWfjA==", (String)null, (String)null);
        try {

            // 请求实体
            InsuranceCustprtlProtocolPayRequestV1 request = new InsuranceCustprtlProtocolPayRequestV1();
            // bizContent实体
            InsuranceCustprtlProtocolPayRequestV1.ApiInvestmentInsuranceCustprtlProtocolPayRequestV1Biz bizContent
                    = new InsuranceCustprtlProtocolPayRequestV1.ApiInvestmentInsuranceCustprtlProtocolPayRequestV1Biz();
            InsuranceCustprtlProtocolPayRequestV1.CommonReqInfo commonReqInfoContent
                    = new InsuranceCustprtlProtocolPayRequestV1.CommonReqInfo();
            InsuranceCustprtlProtocolPayRequestV1.TransReqInfo transReqInfoContent
                    = new InsuranceCustprtlProtocolPayRequestV1.TransReqInfo();
            InsuranceCustprtlProtocolPayRequestV1.BizContentInfo bizContentInfo =
                    new InsuranceCustprtlProtocolPayRequestV1.BizContentInfo();


            commonReqInfoContent.setChanNo("15");
            commonReqInfoContent.setInsuId("028");
            commonReqInfoContent.setTransDate("xxxxxx");
            commonReqInfoContent.setTransTime("191616");
            commonReqInfoContent.setZoneNo("01202");
            commonReqInfoContent.setBrNo("00230");

            transReqInfoContent.setCustName("张三");
            transReqInfoContent.setCustIdType("0");
            transReqInfoContent.setCustIdNo("xxxxxxxxxxxxxxxx");
            transReqInfoContent.setPrtlNo("xxxxxxxxxxxx");
            transReqInfoContent.setInsuPrtlType("1");
            transReqInfoContent.setInsuPrtlNo("xxxxxxxxxxxxx");
            transReqInfoContent.setTransSeq("xxxxxxxxxxx");
            transReqInfoContent.setReptFlag("0");
            transReqInfoContent.setTransAmt("xxxxxxxxxxx");
            transReqInfoContent.setTransNotes("保费代扣");

            bizContentInfo.setCommonReqInfo(commonReqInfoContent);
            bizContentInfo.setTransReqInfo(transReqInfoContent);

            bizContent.setBizContentInfo(bizContentInfo);

            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);

            InsuranceCustprtlProtocolPayResponseV1 response = client.execute(request);
            log.info(response.toString());

            boolean flag=response.isSuccess();
            if (flag) {
                // 业务成功处理
                log.info("success");//
            } else {
                // 失败
                log.info("error");
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
