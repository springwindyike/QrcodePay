package com.icbc.api;

import com.icbc.api.request.MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestV1;
import com.icbc.api.request.MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestV1.MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestBizV1;
import com.icbc.api.response.MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryResponseV1;

public class MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryTest {

    protected static String APP_ID = "10000000000000002225";
    protected static String APIGW_PUBLIC_KEY =
            "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    protected static String PRI_KEY =
            "";
    protected static String BASE_URL = "http://122.19.61.228:8081/api"; // api地址

    public static void main(String[] args) throws Exception {
        String URI = "mybank/account/enterprise/settleaccount/regulation/maesrprotocolpurqry/V1";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        try {
            MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestBizV1 bizContent =
                    new MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestBizV1();
            MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestV1 request = new MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryRequestV1();
            // 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
            bizContent.setfSeqNo("123");
            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);
            MybankAccountEnterpriseSettleaccountRegulationMaesrprotocolpurqryResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println("success");
            } else {
                // 失败
                System.out.println("error");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
