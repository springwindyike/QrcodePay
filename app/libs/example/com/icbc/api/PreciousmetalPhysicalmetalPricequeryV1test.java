package com.icbc.api;

import java.util.ArrayList;
import java.util.List;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.PreciousmetalPhysicalmetalPricequeryRequestV1;
import com.icbc.api.request.PreciousmetalPhysicalmetalPricequeryRequestV1.PreciousmetalPhysicalmetalPricequeryRequestV1Biz;
import com.icbc.api.response.PreciousmetalPhysicalmetalPricequeryResponseV1;

public class PreciousmetalPhysicalmetalPricequeryV1test {

	protected static final String MY_PRIVATE_KEY = "";

	protected static final String APIGW_PUBLIC_KEY = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002166";

	@Test
	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		PreciousmetalPhysicalmetalPricequeryRequestV1 request = new PreciousmetalPhysicalmetalPricequeryRequestV1();
		request.setServiceUrl("http://122.19.77.226:8081/api/preciousmetal/physicalmetal/V1/pricequery");

		PreciousmetalPhysicalmetalPricequeryRequestV1Biz bizContent = new PreciousmetalPhysicalmetalPricequeryRequestV1Biz();
		bizContent.setTrxSeqNo("123456");
		bizContent.setTrxTs("2017-04-07 17:15:12.762126");
		List<Integer> phyPptxnnoList = new ArrayList<Integer>();
		phyPptxnnoList.add(10020);
		phyPptxnnoList.add(10100);
		bizContent.setPhyPptxnnoList(phyPptxnnoList);
	    
		request.setBizContent(bizContent);

		PreciousmetalPhysicalmetalPricequeryResponseV1 response = client.execute(request, "msgId");

		System.out.println(JSONObject.toJSONString(response));
		if (response.isSuccess()) {
			// 业务成功处理
			System.out.println(response.getReturnCode());
		} else {
			// 失败
		}

	}
}

