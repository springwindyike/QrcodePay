package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.request.MybankEnterprisePayQrybankstatementRequestV1;
import com.icbc.api.request.MybankEnterprisePayQrybankstatementRequestV1.MybankEnterprisePayQrybankstatementRequesBizV1;
import com.icbc.api.response.MybankEnterprisePayQrybankstatementResponseV1;
/**
 * �߼��²�ѯ���˵�
 * @author kfzx-yangjiang
 *
 */
public class MybankEnterprisePayQrybankstatementTest {

    protected static String APP_ID = "10000000000000002225";
    protected static String APIGW_PUBLIC_KEY =
            "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCwFgHD4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    protected static String PRI_KEY =
            "";
    protected static String BASE_URL = "http://122.19.61.228:8081/api";

    public static void main(String[] args) throws Exception {
        String URI = "/mybank/enterprise/pay/qrybankstatement/V1"; 
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        try {

            MybankEnterprisePayQrybankstatementRequesBizV1 bizContent = new MybankEnterprisePayQrybankstatementRequesBizV1();
            MybankEnterprisePayQrybankstatementRequestV1 request = new MybankEnterprisePayQrybankstatementRequestV1();
            bizContent.setBankCode("102");
            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);
            MybankEnterprisePayQrybankstatementResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                System.out.println("success");
            } else {
                System.out.println("error");
            }

        } catch (Exception e) {
            e.printStackTrace();
        }


    }
}
